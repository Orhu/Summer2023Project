...
namespace CardSystem.Effects
{
    ...
    {
        [Header("Mechanics")] [Tooltip("The damage that will be dealt to anything this hits.")]
        public Attack attack;

        [Tooltip("Whether or not the range is multiplied by the number stacks when played.")]
        public bool stackAttack = true;

        [Tooltip("The radius of the cone.")] public float range = 1;
        ...
        [Header("Visuals")] [Tooltip("The previewer prefab to use.")]
        ...
        {
            DamageInConePreviewer previewer =
                Instantiate<DamageInConePreviewer>(previewerPrefab, actor.GetActionSourceTransform());
        ...
        {
            if (playersToPreviewers.TryGetValue(actor, out var result))
                CancelPreview(actor);
            ...
            Collider2D[] OverlapingColliders = Physics2D.OverlapCircleAll(actor.GetActionSourceTransform().position,
                range * (stackRange ? numStacks : 1));
            foreach (Collider2D OverlapingCollider in OverlapingColliders)
            {
                ...
                {
                    Vector2 aimDirection = (actor.GetActionAimPosition() - actor.GetActionSourceTransform().position)
                        .normalized;
                    Vector2 overlapDirection =
                        (OverlapingCollider.transform.position - actor.GetActionSourceTransform().position).normalized;

                    if (Vector2.Dot(aimDirection, overlapDirection) >
                        Mathf.Cos(arcWidth * (stackArcWidth ? numStacks : 1) / 2f))
                    {
